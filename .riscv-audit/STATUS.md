...
新增审计条目 2 个；跟踪中的已知开放问题 2 个；已知限制或设计问题 1 个。
...
| [F004](findings/F004-cross-compile-cpuinfo-unguarded.md) | RISC-V 交叉编译使用构建主机 /proc/cpuinfo 判断目标 FP16/BF16 能力 | 静态链高；场景 B 配置复现高 | 场景 B 配置阶段 `confirmed`（[README](validation/F004-cross-compile/README.md) / [REPORT](validation/F004-cross-compile/REPORT.md)）；场景 A 未验证；正式修复尚未设计 |
...
2. F004：基于已确认的场景 B 进入正式修复设计；场景 A、VLEN=256、实际编译与目标运行仍待验证
