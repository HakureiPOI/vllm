...
- 在 `FENV_ACCESS` ON，或编译器以等价方式保留动态浮点环境语义时，若线程当前舍入模式被设置为非 RNE，implicit FP32→FP16/BF16 转换可能产生与固定 RNE 路径不同的结果。
- 仅修改 `fcsr.frm` 并不意味着 C intrinsic 在所有编译模式下都必须观察该变化；`FENV_ACCESS` OFF 时 implicit intrinsic 使用默认浮点环境语义。
- 当前尚未证明 vLLM 的实际构建和运行环境中存在可观察影响。
